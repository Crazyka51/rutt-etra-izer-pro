...
_guiOptions  = {
	...
	depthInvert: 	false, // Inverze hloubky (tmavá = vysoká)
	brightMin: 		0,     // Spodní hranice jasu pro 3D
	brightMax: 		255,   // Horní hranice (ořez odlesků)
	depthFalloff: 	1.0,   // Křivka hloubky (Power)
	// Pokročilé barevné ovládání
	hue: 			0,     // -180 to 180 (barevný posun)
	gamma: 			1.0,   // 0.1 to 3.0 (gamma korekce)
	rGain: 			1.0,   // 0 to 2.0 (zesílení červené)
	gGain: 			1.0,   // 0 to 2.0 (zesílení zelené)
	bGain: 			1.0,   // 0 to 2.0 (zesílení modré)
	invertImage: 	false  // Inverze barev
};
...
$(document).ready( function() {
	...
	log('========================================', 'info');
	log('Rutt-Etra-Izer Pro STARTUP', 'info');
	log('========================================', 'info');
	...
	$("#loadFile").click( async function() {
		try {
			log('Otevírání dialogu pro výběr souboru...', 'info');
			const { ipcRenderer } = require('electron');
			const filePath = await ipcRenderer.invoke('select-file');
			
			if (filePath) {
				log('Vybrán soubor: ' + filePath, 'info');
				const fs = require('fs');
				...
				log('Výběr souboru zrušen', 'info');
			}
		} catch (error) {
			log('CHYBA při načítání souboru: ' + error.message, 'error');
			console.error('Chyba při načítání souboru:', error);
			alert('Nepodařilo se načíst obrázek. Zkuste to prosím znovu.');
		}
	});
...
function createLines() {
	...
	// Vytvoř nový materiál pouze pokud neexistuje nebo byl zničen
	if (!_material) {
		...
		// Aktualizuj existující materiál
	...
	try {
		...
		for(y = 0; y < _imageHeight; y+= _guiOptions.scanStep) {
			...
			for(x = 0; x < _imageWidth ; x+= _guiOptions.scanStep) {
				...
				// Pokročilý výpočet hloubky s ořezem a inverzí
				var pixelBrightness = brightness * 255; // 0-255 škála
				var activeBrightness = pixelBrightness;
				
				// 1. Ořez jasu - ignoruj odlesky mimo rozsah
				if (activeBrightness < _guiOptions.brightMin || activeBrightness > _guiOptions.brightMax) {
					activeBrightness = _guiOptions.brightMin; // Přibi k základní rovině
				}
				
				// Normalizace do rozsahu 0-1 podle nastaveného ořezu
				...
				// 2. Inverze hloubky - tmavé věci vylezou nahoru
				if (_guiOptions.depthInvert) {
					normalizedBrightness = 1.0 - normalizedBrightness;
				}
				
				// 3. Kontrast hloubky (Falloff) - nelineární zvýraznění detailů
				var zDepth = Math.pow(normalizedBrightness, _guiOptions.depthFalloff);
				
				// Skutečná hloubka se aplikuje v render() pomocí scale, zde ukládáme normalizovanou hodnotu
...
function updateImage() {
	...
	// Debounce - počkej 300ms po poslední změně
...
function disposeLineGroup() {
	// Správně uvolni paměť pro staré 3D objekty
	if (_lineGroup) {
		try {
			// Dispose geometrií všech lines
			...
			// Odstranit _lineGroup z _lineHolder
	...
	// NEMAZAT _material - je sdílený napříč všemi voláními createLines
	// Materiál se vytvoří jednou a znovu použije
...
function resetFilters() {
	...
	_guiOptions.bGain = 1.0;
	_guiOptions.invertImage = false;
	_gui.updateDisplay(); // aktualizuje GUI
	updateImage();
}
...
function applyFilters() {
	...
	// Kontrola, zda jsou aktivní nějaké filtry
	...
	// Pokud nejsou filtry, použij rovnou originální pixely
	if (!hasFilters) {
	...
	// 10. Vignette (vyžaduje X,Y souřadnice)
	if (_guiOptions.vignette > 0) {
		applyVignette(data);
	}
	
	// 11. Sharpness (konvoluce)
...
function render() {
	...
	// Aplikuj scale včetně hloubky na Z-ose
	var depthScale = _guiOptions.depth / 100; // Normalizovaná hodnota
	...
	if (_guiOptions.autoRotate) {
		...
		// Manuální rotace myší + posuvníky
